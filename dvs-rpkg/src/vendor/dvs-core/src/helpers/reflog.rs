...
impl<'a> SnapshotStore<'a> {
    ...
    /// Find a snapshot ID by prefix.
    ///
    /// Returns the full ID if exactly one snapshot matches the prefix.
    /// Returns an error if no matches or multiple matches are found.
    pub fn find_by_prefix(&self, prefix: &str) -> Result<String, DvsError> {
        ...
        match matches.len() {
            0 => Err(DvsError::not_found(format!(
                "No snapshot found matching prefix: {}",
                prefix
            ))),
            1 => Ok(matches[0].clone()),
            _ => Err(DvsError::not_found(format!(
                "Ambiguous prefix '{}' matches {} snapshots (be more specific)",
                prefix,
                matches.len()
            ))),
        }
