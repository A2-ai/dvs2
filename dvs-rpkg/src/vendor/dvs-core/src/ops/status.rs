...
pub struct StatusOptions {
    /// When true, verify that storage files exist and have correct hashes.
    ///
    /// This is more expensive as it requires hashing storage files,
    /// but detects storage corruption or missing files.
...
pub fn status_with_backend(
    ...
    status_with_options(backend, files, &StatusOptions::default())
}

/// Check status with a specific backend and options.
///
/// Use this when you need to specify options like storage verification.
pub fn status_with_options(
    ...
    for path in target_files {
        let result = status_single_file(backend, &path, &config, options);
        results.push(result);
    }
...
fn status_single_file(
    ...
    // Determine local file status
    let local_status = match determine_status(path, &metadata) {
        Ok(s) => s,
        Err(e) => {
            return StatusResult::error(
                path.display().to_string(),
    ...
    // Compute storage path (using algorithm from metadata)
    ...
    let status = if !storage_path.exists() {
        // Storage file missing
        if local_status == FileStatus::Unsynced {
            // Local file is modified, so storage missing is expected
            // (the old version may not be in storage)
            local_status
        } else {
            // Storage should exist but doesn't - this is an integrity issue
            FileStatus::StorageMissing
        }
    } else if options.verify_storage && local_status == FileStatus::Current {
        // Verify storage integrity by hashing the storage file
        match verify_storage_integrity(&storage_path, &metadata) {
            Ok(true) => local_status,
            Ok(false) => FileStatus::StorageCorrupted,
            Err(_) => {
                // If we can't verify, return as error
                return StatusResult::error(
                    path.display().to_string(),
                    "storage_verify_error".to_string(),
                    format!("Failed to verify storage file: {}", storage_path.display()),
                );
            }
        }
    } else {
...
/// Verify that the storage file matches the expected hash.
fn verify_storage_integrity(storage_path: &Path, metadata: &Metadata) -> Result<bool, DvsError> {
    hash::verify_hash_with_algo(storage_path, &metadata.blake3_checksum, metadata.hash_algo)
}
...
mod tests {
    ...
    fn test_status_current() {
        ...
        let result = status_single_file(&backend, &test_file, &config, &StatusOptions::default());
