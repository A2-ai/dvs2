...
impl LocalConfig {
    ...
    /// Fallback load when toml-config is not available.
    ///
    /// Always returns default config (local config not supported without toml-config).
    #[cfg(not(feature = "toml-config"))]
    pub fn load(_path: &Path) -> Result<Self, DvsError> {
        Ok(Self::default())
    }
