...
pub enum FileStatus {
    ...
    Unsynced,
    /// Storage file doesn't exist (integrity issue).
    StorageMissing,
    /// Storage file exists but hash mismatch (integrity issue).
    StorageCorrupted,
...
impl FileStatus {
    /// Returns true if this status indicates an integrity problem.
    ///
    /// Integrity issues mean that the storage is corrupted or missing,
    /// which is a more serious problem than a local file being modified.
    pub fn is_integrity_issue(&self) -> bool {
        matches!(self, Self::StorageMissing | Self::StorageCorrupted)
    }
}
