...
dvs_add_impl <- function(files = character(0), message = NULL, glob = NULL, dry_run = NULL, progress_callback = NULL, num_threads = NULL) {
  stopifnot(
    ...
    "'num_threads' must be NULL or numeric, logical, or raw" = is.null(num_threads) || is.numeric(num_threads) || is.logical(num_threads) || is.raw(num_threads),
    "'num_threads' must be NULL or have length 1" = is.null(num_threads) || length(num_threads) == 1L,
    "'num_threads' must be NULL or non-negative" = is.null(num_threads) || is.raw(num_threads) || num_threads >= 0
  )
  .val <- .Call(C_dvs_add, .call = match.call(), files, message, glob, dry_run, progress_callback, num_threads)
  if (inherits(.val, "rust_error_value") && isTRUE(attr(.val, "__rust_error__"))) {
    stop(structure(
      class = c("rust_error", "simpleError", "error", "condition"),
      list(message = .val$error, call = .val$call %||% sys.call(), kind = .val$kind)
...
dvs_get_impl <- function(files = character(0), glob = NULL, dry_run = NULL, progress_callback = NULL, num_threads = NULL) {
  stopifnot(
    "'glob' must be NULL or character" = is.null(glob) || is.character(glob),
    "'glob' must be NULL or have length 1" = is.null(glob) || length(glob) == 1L,
    "'dry_run' must be NULL or logical" = is.null(dry_run) || is.logical(dry_run),
  ...
  .val <- .Call(C_dvs_get, .call = match.call(), files, glob, dry_run, progress_callback, num_threads)
  if (inherits(.val, "rust_error_value") && isTRUE(attr(.val, "__rust_error__"))) {
    stop(structure(
      class = c("rust_error", "simpleError", "error", "condition"),
      list(message = .val$error, call = .val$call %||% sys.call(), kind = .val$kind)
...
dvs_status_impl <- function(current = NULL, absent = NULL, unsynced = NULL, num_threads = NULL) {
  stopifnot(
  ...
  .val <- .Call(C_dvs_status, .call = match.call(), current, absent, unsynced, num_threads)
  if (inherits(.val, "rust_error_value") && isTRUE(attr(.val, "__rust_error__"))) {
    stop(structure(
      class = c("rust_error", "simpleError", "error", "condition"),
      list(message = .val$error, call = .val$call %||% sys.call(), kind = .val$kind)
