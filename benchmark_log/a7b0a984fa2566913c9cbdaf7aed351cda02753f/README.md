# Benchmark: a7b0a98 (with "Fix save to not read" optimization)

## Code state

Branch: `vvv`
Built from commit `ec1ca91`, rebased onto `a7b0a98`.
Includes PR #68 (`36ff015 Fix save to not read`) which removes the
pre-write `backend.read()` call — storage writes no longer read old
